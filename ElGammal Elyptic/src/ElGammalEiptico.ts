...
export class ElGammalEiptico {
  ...
  constructor(p_: number, a_: number, b_: number, G_: Point, private da: number, private db: number, m_: number) {
    ...
    const diffieA = new DH(this.da, this.G, this.a, this.p);
    const diffieB = new DH(this.db, this.G, this.a, this.p);
    ...
    const diffieSa = new DH(this.da, this.publicB, this.a, this.p);
    const diffieSb = new DH(this.db, this.publicA, this.a, this.p);
...
let b: number = 3;
let G: string = '9,6';
let point: string[] = G.split(',');
const PointG: Point = [parseInt(point[0]), parseInt(point[1])];
let da: number = 4;
let db: number = 2;
let m: number = 2;

const cypher = new ElGammalEiptico(p, a, b, PointG, da, db, m);
cypher.print()