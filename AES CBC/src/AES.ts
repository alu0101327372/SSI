...
export class AES {
  ...
  */
  constructor(key: string, text: string) {
    ...
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
        this.key[i][j] = key.slice(k, l);
        k += 2; l += 2;
      }
    }
    k = 0; l = 2;
    this.key = this.transpose(this.key);
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
        this.text[i][j] = text.slice(k, l);
        k += 2; l += 2;
      }
    }
    this.text = this.transpose(this.text);
  }
  ...
  */
  protected encrypt(): string[][] {
    ...
    // console.log(`\nRC0  (SubClave = ${this.transpose(keys[0])}) = ${this.transpose(this.cipherText)}`);
    // 9 iteraciones
    for (let i = 1; i < this.numOfRounds; i++) {
      ...
      // console.log(`RC${i} (SubClave = ${this.transpose(keys[i])}) = ${this.transpose(this.cipherText)}`)
    }
    // Etapa final
    this.SubBytes();
    this.shiftRows();
    this.addRoundKey(keys[10], this.cipherText);
    // console.log(`RC10 (SubClave = ${this.transpose(keys[keys.length - 1])}) = ${this.transpose(this.cipherText)}`)
   ...
   */
   private addRoundKey(key: string[][], text: string[][]): void {
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
  ...
  */
  private SubBytes(): void {
    let rowBox = 0; let colBox = 0;
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
  ...
  */
   private rotateLeft(input: string[], rotate: number): string[] {
  ...
  * @returns
  */
  private multiplyMatrixColumn(column: string[]): string[] {
    ...
    for (let i = 0; i < this.M_mixColum.length; i++) {
      for (let j = 0; j < this.M_mixColum.length; j++) {
        ...
        const output = new Multiply(this.M_mixColum[i][j], parseInt(column[j], 16));
  ...
  */
  private getExpandKey(iExpKey: string[][], index: number): string[][] {
    const word = this.transpose(iExpKey);
    let temp: string[] = word[3];
    for (let i = 0; i < 4; i++) {
      if (i + 4 % 4 == 0) {
        ...
        for (let j = 0; j < 4; j++) {
          ...
          let firstxor = this.xorBit2Bit(firstByte, secondByte);
          let RcByte = parseInt(this.Rc[index-1][j].toString(16),16).toString(2).padStart(8, '0');
          temp[j] = this.xorBit2Bit(firstxor, RcByte)
        }
        for (let j = 0; j < 4; j++) {
        ...
        for (let j = 0; j < 4; j++) {
          let firstByte = parseInt(word[i][j], 16).toString(2).padStart(8, '0');
          let secondByte = parseInt(temp[j], 16).toString(2).padStart(8, '0');
          temp[j] = this.xorBit2Bit(firstByte, secondByte)
        }
        for (let j = 0; j < 4; j++) {
          temp[j] = parseInt(temp[j], 2).toString(16).padStart(2, '0');
        }
  ...
  */
  private SubBytes_ExpKey(temp: string[]): string[] {
    let rowBox = 0, colBox = 0;
    for (let i = 0; i < 4; i++) {
   ...
   */
   protected transpose(matrix: string[][]) {
    let aux: string[][] =  [[], [], [], []];
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 4; j++) {
        aux[j][i] = matrix[i][j];
