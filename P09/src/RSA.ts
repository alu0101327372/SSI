...
export class RSA {
   ...
   */
   fastExponentiation(base: number, exponent: number, module: number): number {
    let x = 1;
    let y = base % module;
   ...
   */
   eucExt(a: number, module: number): number {
    // Inicalizar valores
   ...
   * @returns {number}
   */
   blocksize(b: number, n: number): number {
...
var text = scanf('%s');
console.log('Introduce el número p: ');
var p = scanf('%d');
console.log('Introduce el número q: ');
var q = scanf('%d');
console.log('Introduce el número d: ');
var d = scanf('%d');
*/


const cypher = new RSA('MANDA DINEROS', 421, 7, 1619);
cypher.printRSA()